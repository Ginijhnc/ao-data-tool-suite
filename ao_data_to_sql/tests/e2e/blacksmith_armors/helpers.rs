//! Helper functions for blacksmith armor E2E tests.
//!
//! Provides utilities to verify blacksmith armor count and data against fixtures.
...
/// Helper that verifies the number of blacksmith armors in the database.
pub async fn verify_blacksmith_armor_count(expected_count: i64) {
    ...
    assert_eq!(
        ...
        "Expected {} blacksmith armors, found {}",
        expected_count, count.0
    );
}

/// Helper that verifies blacksmith armor data matches a JSON fixture.
pub async fn verify_blacksmith_armor_data_matches_fixture(
    ...
    let (actual_data,): (serde_json::Value,) =
        sqlx::query_as("SELECT data FROM blacksmith_armors WHERE id = $1")
            ...
            .fetch_one(&pool)
            .await
            .expect("failed to fetch blacksmith armor data");
    ...
    assert_eq!(
        ...
        "Blacksmith armor {armor_id} data does not match fixture"
    );
}
