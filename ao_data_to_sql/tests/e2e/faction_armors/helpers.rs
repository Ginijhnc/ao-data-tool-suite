//! Helper functions for faction armor E2E tests.
//!
//! Provides utilities to verify faction armor count and data against fixtures.
...
/// Helper that verifies the number of faction armor classes in the database.
pub async fn verify_faction_armor_count(expected_count: i64) {
    ...
    let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM faction_armors")
        .fetch_one(&pool)
        .await
    ...
    assert_eq!(
        ...
        "Expected {} faction armors, found {}",
        expected_count, count.0
    );
}

/// Helper that verifies faction armor data matches a JSON fixture.
pub async fn verify_faction_armor_data_matches_fixture(
    ...
    let (actual_data,): (serde_json::Value,) =
        sqlx::query_as("SELECT data FROM faction_armors WHERE id = $1")
            .bind(armor_id)
            .fetch_one(&pool)
            .await
            .expect("failed to fetch faction armor data");
    ...
    assert_eq!(
        ...
        "Faction armor {armor_id} data does not match fixture"
    );
}
