...
pub const OBJ_FIXTURES: &str = "objects";

/// Helper that verifies the number of objects in the database.
pub async fn verify_object_count(expected_object_count: i64) {
    ...
    assert_eq!(
        count.0, expected_object_count,
        "Expected {} objects in objects table, found {}",
        expected_object_count, count.0
    );
}

/// Helper that verifies object data matches a JSON fixture.
pub async fn verify_object_data_matches_fixture(
    ...
    let (actual_data,): (serde_json::Value,) =
        sqlx::query_as("SELECT data FROM objects WHERE id = $1")
            .bind(object_id)
            .fetch_one(&pool)
            .await
            .expect("failed to fetch object data");
    ...
    assert_eq!(
        ...
        "Object {object_id} data does not match fixture"
    );
}
