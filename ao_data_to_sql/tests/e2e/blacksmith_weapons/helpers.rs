//! Helper functions for blacksmith weapon E2E tests.
//!
//! Provides utilities to verify blacksmith weapon count and data against fixtures.
...
/// Helper that verifies the number of blacksmith weapons in the database.
pub async fn verify_blacksmith_weapon_count(expected_count: i64) {
    ...
    assert_eq!(
        ...
        "Expected {} blacksmith weapons, found {}",
        expected_count, count.0
    );
}

/// Helper that verifies blacksmith weapon data matches a JSON fixture.
pub async fn verify_blacksmith_weapon_data_matches_fixture(
    ...
    let (actual_data,): (serde_json::Value,) =
        sqlx::query_as("SELECT data FROM blacksmith_weapons WHERE id = $1")
            ...
            .fetch_one(&pool)
            .await
            .expect("failed to fetch blacksmith weapon data");
    ...
    assert_eq!(
        ...
        "Blacksmith weapon {weapon_id} data does not match fixture"
    );
}
