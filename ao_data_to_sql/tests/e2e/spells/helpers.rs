//! Helper functions for spell E2E tests.
//!
//! Provides utilities to verify spell count and data against fixtures.
...
/// Helper that verifies the number of spells in the database.
pub async fn verify_spell_count(expected_spell_count: i64) {
    ...
    assert_eq!(
        count.0, expected_spell_count,
        "Expected {} spells in spells table, found {}",
        expected_spell_count, count.0
    );
}

/// Helper that verifies spell data matches a JSON fixture.
pub async fn verify_spell_data_matches_fixture(
    ...
    let (actual_data,): (serde_json::Value,) =
        sqlx::query_as("SELECT data FROM spells WHERE id = $1")
            .bind(spell_id)
            .fetch_one(&pool)
            .await
            .expect("failed to fetch spell data");
    ...
    assert_eq!(
        ...
        "Spell {spell_id} data does not match fixture"
    );
}
